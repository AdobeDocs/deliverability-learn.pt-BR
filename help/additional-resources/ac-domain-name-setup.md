...
Ao delegar um subdomínio para uso com o Adobe Campaign, os clientes podem confiar na Adobe para manter a infraestrutura de DNS necessária para atender aos requisitos de capacidade de entrega padrão do setor para seus domínios de envio de marketing por email, enquanto continuam a manter e controlar o DNS para seus domínios de email internos.  A delegação de subdomínio permite:

Clientes para manter a imagem da marca usando um alias DNS com seus nomes de domínio
A Adobe implementará de forma autônoma todas as práticas recomendadas técnicas para otimizar totalmente a capacidade de entrega durante o envio por email

## Opções de configuração de DNS

Para fornecer um serviço gerenciado baseado em nuvem, a Adobe incentiva os clientes a usar a delegação de subdomínio ao implantar o Adobe Campaign.  No entanto, a Adobe oferece aos clientes uma opção alternativa - configuração CNAME - para configurar o DNS.

| Opção | Descrição | Responsabilidades da Adobe | Responsabilidades do cliente |
|--- |------- |--- |--- |
| Delegação de subdomínio para o Adobe Campaign | O cliente delega um subdomínio (email.example.com) ao Adobe. Nesse cenário, o Adobe pode fornecer o Campaign como um serviço gerenciado controlando e mantendo todos os aspectos do DNS necessários para fornecer, renderizar e rastrear campanhas de email. | Gerenciamento completo do subdomínio e de todos os registros DNS necessários para o Adobe Campaign. | Delegação adequada do subdomínio à Adobe |
...
| DKIM (TXT) | Email identificado de DomainKeys | <i>client._domainkey.email.example.com</i></br>&quot;v=DKIM1; k=rsa;&quot; &quot;DKIMPUBLICKEY AQUI&quot; |
...
| DNS reverso (PTR) | Mapeia os endereços IP do cliente para um nome de host da marca do cliente | Ponteiro de nome de domínio 18.101.100.192.in-addr.arpa r18.email.example.com |
...
A Autenticação de mensagens, Relatórios e Conformidade (DMARC) baseados em domínio é recomendada para autenticar remetentes de email e garantir que os sistemas de email de destino confiem em mensagens enviadas do seu domínio.
...
Você pode implementar o DMARC manualmente ou entrar em contato com a Adobe para ajudá-lo a configurar o DMARC para sua marca.

## Requisitos de configuração

### Delegação de subdomínio

Isso requer que o cliente crie um subdomínio em seus servidores DNS e defina os servidores de nomes para que esse subdomínio seja mantido pela Adobe.  Por exemplo, um cliente cujo nome de domínio principal seja &quot;example.com&quot; e que deseje delegar o gerenciamento de &quot;marketing.example.com&quot; ao Adobe para seus deliveries de email terá que materializar essa delegação para adicionar os seguintes registros de tipo ao seu DNS:
...
Se o cliente optar por usar CNAMEs em vez de delegar um subdomínio à Adobe, durante a fase de configuração, a Adobe fornecerá os registros a serem colocados nos servidores DNS do cliente e configurará os valores correspondentes nos servidores DNS da Adobe Campaign.
...
| Subdomínio | Do endereço | Do nome | Endereço para resposta |
|--- |--- |--- |--- |
...
Assim que os subdomínios de envio de email forem delegados corretamente na Adobe Campaign, a equipe de TechOps da Adobe criará dois ou mais domínios de nível inferior para gerenciar páginas de rastreamento e mirror de maneira independente.
...
Após essas delegações, a infraestrutura implementada pela Adobe garante que os seguintes serviços sejam executados para cada domínio de envio delegado ou com alias CNAME:

* Criação de caixas de entrada postmaster@ e abuse@
* Configuração de loops de comentários para o domínio delegado
* Mediante solicitação, o Adobe também configurará um registro DMARC conforme especificado. Seu consultor de capacidade de entrega pode ajudar a projetar uma política e um plano de longo prazo do DMARC para seus domínios de envio.
Os parâmetros estabelecidos pela Adobe são válidos somente a partir do momento em que a delegação foi concluída e, em seguida, verificada pela Adobe, e permanecem funcionais.  Todas as ofertas da Adobe Campaign Cloud incluem a delegação de nome de domínio como padrão.
...
Se isso acontecer, a Adobe fornecerá ao CLIENTE uma estimativa detalhando o número de dias de serviço necessários para retornar ao modo de não delegação de domínio.

A Adobe será exonerada de qualquer responsabilidade pelo compromisso da Taxa de capacidade de entrega acima mencionada se o Cliente não cumprir os compromissos definidos acima.

O encerramento do Serviço do Marketing Cloud resultará automaticamente no fim das delegações de domínio e na manutenção de DNS para esses domínios pela Adobe.
